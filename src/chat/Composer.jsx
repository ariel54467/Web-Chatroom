...
export function Composer({ roomId, userId, disabled, onSent, onError }) {
  ...
  function choose(next) {
    if (!next) return;
    try { validateFile(next); setFile(next); setGif(null); setError(""); } catch (e) { setError(e.message); }
  }
  function chooseGif(next) {
    setGif(next); setFile(null); setError("");
  }
  ...
  async function discard() {
    ...
    if (mounted.current) { setPhase(""); setFile(null); setGif(null); setError(""); }
  }
  async function send(event) {
    event?.preventDefault();
    if (busy.current || disabled || (!body.trim() && !file && !gif && !attempt.current)) return;
    busy.current = true; setError("");
    const pending = attempt.current || { id: crypto.randomUUID(), body: body.trim(), file, gif,
      kind: file ? validateFile(file) : gif ? "gif" : "text", externalUrl: gif?.url || null, uploaded: false };
    ...
    try {
      ...
      const message = await rpc("send_message", {
        ...
        p_storage_path: pending.path || null, p_file_name: pending.file?.name || pending.gif?.title || null,
        p_external_url: pending.externalUrl,
      });
      attempt.current = null;
      try { localStorage.removeItem(key); } catch { /* Storage can be disabled. */ }
      if (mounted.current) {
        setBody(""); setFile(null); setGif(null); setPhase(""); onSent(message);
  ...
  return <div className="composer-area">
    {(file || gif) && <div className="file-preview">
      {file?.type === "video/mp4" ? <Film size={30} /> : <img src={gif?.preview || gif?.url || preview} alt="Attachment preview" />}
      <span><strong>{gif?.title || file?.name || "GIF"}</strong><small>{gif ? "GIPHY" : `${(file.size / 1024 / 1024).toFixed(2)} MB`}</small></span>
    ...
    <form className="composer" onSubmit={send}>
      ...
      <IconButton label="Choose a GIF" disabled={disabled || locked} onClick={() => setGifOpen(true)}><Smile size={20} /></IconButton>
      ...
      <button type="submit" className="send-button" aria-label="Send message" title="Send message" disabled={disabled || locked || (!body.trim() && !file && !gif)}>
        {locked && phase !== "failed" ? <Busy /> : <Send size={19} />}
      </button>
    </form>
    {gifOpen && <GifPicker onClose={() => setGifOpen(false)} onChoose={next => { chooseGif(next); setGifOpen(false); }} onUpload={() => { setGifOpen(false); uploadInput.current?.click(); }} />}
  </div>;
}
